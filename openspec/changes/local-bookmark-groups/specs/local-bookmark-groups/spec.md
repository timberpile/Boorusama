...
The bookmarks page SHALL provide a group selector containing the `All Bookmarks` and `Ungrouped` system views and all user-created groups.

`All Bookmarks` SHALL show every local bookmark. `Ungrouped` SHALL show only bookmarks with no named group memberships.

#### Scenario: View all bookmarks

- **WHEN** the user selects `All Bookmarks`
- **THEN** the page SHALL show every local bookmark regardless of group membership

#### Scenario: View ungrouped bookmarks

- **WHEN** the user selects `Ungrouped`
...
- **THEN** the system SHALL delete the group, remove its memberships, and show those bookmarks in `Ungrouped`
...
Deleting a named group SHALL require confirmation whenever it would remove memberships or bookmarks.

The confirmation SHALL explain how many bookmarks are affected and SHALL offer separate choices to keep orphaned bookmarks as ungrouped or delete them.
...
- **THEN** the system MAY use a simplified confirmation, but SHALL not delete any bookmarks

### Requirement: Active bookmark target

The system SHALL maintain a persisted active bookmark target, defaulting to `Ungrouped`.

Opening a named group or creating a group SHALL make that group the active target. Selecting `All Bookmarks` SHALL leave the active target unchanged. Selecting `Ungrouped` SHALL make `Ungrouped` the active target.
...
- **WHEN** the user has not selected a named group
- **THEN** a single tap on an unbookmarked post SHALL create an ungrouped bookmark

#### Scenario: Persist the active target

- **WHEN** the application is restarted after a user selected a valid target group
- **THEN** the active target SHALL be restored when that group still exists; otherwise it SHALL fall back to `Ungrouped`
...
A long press on the bookmark button SHALL open a group-selection interface. The interface SHALL list available add targets and SHALL provide an option to create a new group.
...
- **THEN** the bookmark SHALL remain locally bookmarked and SHALL appear in `Ungrouped`
...
#### Scenario: Avoid clearing memberships through Ungrouped

- **WHEN** a bookmarked post belongs to one or more named groups and `Ungrouped` is the active target
...
When the bookmark belongs to one or more named groups other than the active target, the button SHALL also show a count badge containing the total number of named groups containing the bookmark.
...
The bookmark section SHALL provide `Add to...`, `Add to <active target>` when applicable, `Remove from...`, `Remove from <active target>` when applicable, and `Delete bookmark completely` when a local bookmark exists.
