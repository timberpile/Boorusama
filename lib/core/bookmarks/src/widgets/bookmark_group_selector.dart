...
import '../providers/bookmark_group_deletion.dart';
...
class BookmarkGroupManagementButton extends ConsumerWidget {
  ...
  Future<void> _deleteGroup(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      await deleteBookmarkGroupAndRefresh(
        deleteGroup: () => repository.deleteGroup(group.id),
        removeOrphanBookmarks: (orphanIds) =>
            ref.read(bookmarkProvider.notifier).removeBookmarksByIds(orphanIds),
        ...
        refreshProviders: () => refreshBookmarkGroupProviders(ref),
      );
    } catch (error) {
      if (!context.mounted) return;
      _showError(context, error);
