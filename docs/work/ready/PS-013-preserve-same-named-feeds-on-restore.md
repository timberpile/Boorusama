...
- [ ] A backup round trip preserves both same-named feeds, their IDs, and their
  respective source queries, including names differing only in case.
- [ ] Repeated import does not duplicate either feed or its hidden sources.
- [ ] Existing profile mapping and cross-profile ID collision handling remain
  correct, without overwriting unrelated local feeds.
- [ ] Observable regression coverage includes the same-name round trip and
  repeated import.
