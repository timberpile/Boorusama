...
fn main() {
    ...
    let mut cmake_out_dir = out_dir.clone();
    for _ in 0..5 {
        assert!(cmake_out_dir.pop(), "unexpected Cargo OUT_DIR layout");
    }
    cmake_out_dir = cmake_out_dir.join("cmake").join(&target).join(&profile);
    ...
    let dav1d_library = find_library_recursive(&installed, dav1d_library_name(&target))
        .unwrap_or_else(|| {
            panic!(
    ...
    let libyuv_library = find_library_recursive(&installed, libyuv_library_name(&target))
        .unwrap_or_else(|| {
            panic!(
...
fn dav1d_library_name(target: &str) -> &'static str {
    if target.contains("msvc") {
        // Meson produces libdav1d.a on Windows; LocalDav1d.cmake has the
        // same fallback for the local dav1d dependency.
        "libdav1d.a"
    } else {
        "libdav1d.a"
    }
